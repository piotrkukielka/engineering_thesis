...
secondstep_krk <- c(data_monthly$dexcess_krk, data_monthly$dexcess_krk[length(data_monthly$dexcess_krk)])
...
secondstep_kw <- c(data_monthly$dexcess_kw, data_monthly$dexcess_kw[length(data_monthly$dexcess_kw)])
...
dexcessvstime <-ggplot(data) + 
  geom_line(data = data[!is.na(data$dexcess_krk),], 
            aes(date, dexcess_krk, color="krk"),
            size=0.1, alpha=0.3) +
  geom_point(aes(date, dexcess_krk, color="krk"), na.rm = TRUE) +
  geom_line(data = data[!is.na(data$dexcess_kw),], 
            aes(date, dexcess_kw, color="kw"),
            size=0.1, alpha=0.3) +
  geom_point(aes(date, dexcess_kw, color="kw"), na.rm = TRUE) +
  ...
  xlab("Data") +
  ylab(as.expression(expression("d-excess [\u2030]"))) + 
...
dexcessvstime
ggsave(plot=dexcessvstime, filename = "plots/dexcess_vs_time.pdf",
       width = 30, height = 30, units = "cm", dpi = 600, device = cairo_pdf)

# WIP
#dexcessvstime <-ggplot(data) + geom_line(data = data[!is.na(data$dexcess_krk),], 
#            aes(date, dexcess_krk, color="krk"),
#            size=0.1) +
#  geom_point(aes(date, dexcess_krk, color="krk"), na.rm = TRUE) +
#  geom_line(data = data[!is.na(data$dexcess_kw),], 
